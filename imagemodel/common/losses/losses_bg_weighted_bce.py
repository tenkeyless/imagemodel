...
def get_bg_weights(
    ...
    cal_bg_to_bg = K.all(
        K.equal(yt_yp, K.ones_like(yt_yp) * [0.0, 0.0]),
        axis=-1,
        keepdims=True,
    )
    ...
    # BG(t) to FG(p)
    cal_bg_to_fg = K.all(
        K.equal(yt_yp, K.ones_like(yt_yp) * [0.0, 1.0]),
        axis=-1,
        keepdims=True,
    )
    ...
    # FG(t) to BG(p)
    cal_fg_to_bg = K.all(
        K.equal(yt_yp, K.ones_like(yt_yp) * [1.0, 0.0]),
        axis=-1,
        keepdims=True,
    )
    ...
    # FG(t) to FG(p)
    cal_fg_to_fg = K.all(
        K.equal(yt_yp, K.ones_like(yt_yp) * [1.0, 1.0]),
        axis=-1,
        keepdims=True,
    )
