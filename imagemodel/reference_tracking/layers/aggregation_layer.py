...
class AggregationLayer(Layer):
    ...
    def __init__(self, bin_size: int, k_size: int = 5, aggregate_mode: str = "weighted_sum", **kwargs):
    ...
    def get_config(self):
        ...
        config.update({"bin_size": self.bin_size, "k_size": self.k_size, "aggregate_mode": self.aggregate_mode})
        return config
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.aggregate_mode == "weighted_sum":
            ...
            ref_value_stacked = tf.reshape(
                    ref_value_stacked,
                    (-1, self.h_size, self.h_size, self.k_size * self.k_size, self.bin_size))
            attn2 = tf.one_hot(tf.argmax(attn, axis=-1), depth=tf.shape(attn)[-1], dtype=tf.float32)
            ...
            raise ValueError("`aggregate_mode` value is not valid. Should be one of 'weighted_sum', 'argmax'.")
        return aggregation
