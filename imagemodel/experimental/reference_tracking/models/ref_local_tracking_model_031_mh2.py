...
from imagemodel.experimental.reference_tracking.layers.ref_local_layer7_mh2 import RefLocal7MH
from imagemodel.experimental.reference_tracking.layers.ref_local_layer8_mh2 import RefLocal8MH
from imagemodel.experimental.reference_tracking.layers.shrink_layer import ShrinkLayer

check_first_gpu()


class RefLocalTrackingModel031MH2ArgumentsDict(TypedDict):
...
class RefLocalTrackingModel031MH2Arguments(ModelArguments[RefLocalTrackingModel031MH2ArgumentsDict]):
    def __init__(self, dic: RefLocalTrackingModel031MH2ArgumentsDict):
        self.dic = dic
    ...
    @classmethod
    def convert_str_dict(cls, string_dict: Dict[str, str]) -> RefLocalTrackingModel031MH2ArgumentsDict:
    ...
    def aggregation_mode(self) -> Optional[str]:
        return self.dic.get('aggregation_mode')
...
class RefLocalTrackingModel031MH2Manager(
        CommonModelManager,
        CommonModelManagerDictGeneratable[RefLocalTrackingModel031MH2ArgumentsDict]):
    def __init__(
            self,
        ...
        __model_default_values = RefLocalTrackingModel031MH2Arguments(__model_default_args)
        ...
        self.aggregation_mode: str = aggregation_mode or __model_default_values.aggregation_mode
    
    @classmethod
    def init_with_dict(cls, option_dict: Optional[RefLocalTrackingModel031MH2ArgumentsDict] = None):
        if option_dict is not None:
            ref_local_tracking_model_031_arguments = RefLocalTrackingModel031MH2Arguments(option_dict)
            return cls(
    ...
    def init_with_str_dict(cls, option_str_dict: Optional[Dict[str, str]] = None):
        if option_str_dict is not None:
            unet_level_arguments_dict = RefLocalTrackingModel031MH2Arguments.convert_str_dict(option_str_dict)
    ...
    def ref_local_tracking_model_031_mh(
            ...
            head_num: int = 4,
            aggregation_mode: str = "average") -> Model:
        ...
        # First
        diff_local1: Layer = RefLocal8MH(
                mode="dot",
                k_size=5,
                intermediate_dim=256,
                head_num=head_num,
                aggregation_mode=aggregation_mode)([main_unet[0], ref_unet[0]])
        ...
        # Second
        diff_local2: Layer = RefLocal7MH(
                mode="dot",
                k_size=5,
                intermediate_dim=128,
                head_num=head_num,
                aggregation_mode=aggregation_mode)([main_unet[1], ref_unet[1]])
        ...
        # Third
        diff_local3: Layer = RefLocal7MH(
                mode="dot",
                k_size=5,
                intermediate_dim=64,
                head_num=head_num,
                aggregation_mode=aggregation_mode)([main_unet[2], ref_unet[2]])
        ...
        # Fourth
        diff_local4: Layer = RefLocal7MH(
                mode="dot",
                k_size=5,
                intermediate_dim=32,
                head_num=head_num,
                aggregation_mode=aggregation_mode)([main_unet[3], ref_unet[3]])
