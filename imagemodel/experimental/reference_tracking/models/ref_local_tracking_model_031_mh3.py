...
class RefLocalTrackingModel031MH3ArgumentsDict(TypedDict):
...
class RefLocalTrackingModel031MH3Arguments(ModelArguments[RefLocalTrackingModel031MH3ArgumentsDict]):
    def __init__(self, dic: RefLocalTrackingModel031MH3ArgumentsDict):
        self.dic = dic
    ...
    @classmethod
    def convert_str_dict(cls, string_dict: Dict[str, str]) -> RefLocalTrackingModel031MH3ArgumentsDict:
...
class RefLocalTrackingModel031MH3Manager(
        CommonModelManager,
        CommonModelManagerDictGeneratable[RefLocalTrackingModel031MH3ArgumentsDict]):
    def __init__(
            self,
        ...
        __model_default_args = get_default_args(self.ref_local_tracking_model_031_mh3)
        __model_default_values = RefLocalTrackingModel031MH3Arguments(__model_default_args)
    ...
    @classmethod
    def init_with_dict(cls, option_dict: Optional[RefLocalTrackingModel031MH3ArgumentsDict] = None):
        if option_dict is not None:
            ref_local_tracking_model_031_arguments = RefLocalTrackingModel031MH3Arguments(option_dict)
            return cls(
    ...
    def init_with_str_dict(cls, option_str_dict: Optional[Dict[str, str]] = None):
        if option_str_dict is not None:
            unet_level_arguments_dict = RefLocalTrackingModel031MH3Arguments.convert_str_dict(option_str_dict)
            return cls.init_with_dict(unet_level_arguments_dict)
        else:
            return cls()
    
    def setup_model(self) -> Model:
        return self.ref_local_tracking_model_031_mh3(
    ...
    # noinspection DuplicatedCode
    @staticmethod
    def ref_local_tracking_model_031_mh3(
        ...
        for head_index in range(head_num):
            intermediate_dim = 256 // head_num
            diff_local1: Layer = RefLocal8(mode="dot", k_size=5, intermediate_dim=intermediate_dim)(
            ...
            agg1s.append(diff_agg1)
        
        agg1 = concatenate(agg1s)
        agg1 = Conv2D(
                filters=bin_num,
                kernel_size=1,
                padding="same",
                use_bias=False,
                kernel_initializer="he_normal")(agg1)
        up1: Layer = __aggregation_up(agg1, filters=bin_num)
        
        # Second
        agg2s = []
        for head_index in range(head_num):
            intermediate_dim = 128 // head_num
            diff_local2: Layer = RefLocal7(mode="dot", k_size=5, intermediate_dim=intermediate_dim)(
            ...
            agg2s.append(diff_agg2)
        
        agg2 = concatenate(agg2s)
        agg2 = Conv2D(
                filters=bin_num,
                kernel_size=1,
                padding="same",
                use_bias=False,
                kernel_initializer="he_normal")(agg2)
        up2: Layer = __aggregation_up(agg2, filters=bin_num)
        
        # Third
        agg3s = []
        for head_index in range(head_num):
            intermediate_dim = 64 // head_num
            diff_local3: Layer = RefLocal7(mode="dot", k_size=5, intermediate_dim=intermediate_dim)(
            ...
            agg3s.append(diff_agg3)
        
        agg3 = concatenate(agg3s)
        agg3 = Conv2D(
                filters=bin_num,
                kernel_size=1,
                padding="same",
                use_bias=False,
                kernel_initializer="he_normal")(agg3)
        up3: Layer = __aggregation_up(agg3, filters=bin_num)
        ...
        for head_index in range(head_num):
            ...
            agg4s.append(diff_agg4)
        
        agg4 = concatenate(agg4s)
        agg4 = Conv2D(
                filters=bin_num,
                kernel_size=1,
                padding="same",
                use_bias=False,
                kernel_initializer="he_normal")(agg4)
