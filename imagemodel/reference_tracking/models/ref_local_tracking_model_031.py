...
from imagemodel.reference_tracking.layers.ref_local_layer7 import RefLocal7
from imagemodel.reference_tracking.layers.ref_local_layer8 import RefLocal8
from imagemodel.reference_tracking.layers.shrink_layer import ShrinkLayer

check_first_gpu()


class RefLocalTrackingModel031ArgumentsDict(TypedDict):
...
class RefLocalTrackingModel031Arguments(ModelArguments[RefLocalTrackingModel031ArgumentsDict]):
    def __init__(self, dic: RefLocalTrackingModel031ArgumentsDict):
        self.dic = dic
    ...
    @classmethod
    def convert_str_dict(cls, string_dict: Dict[str, str]) -> RefLocalTrackingModel031ArgumentsDict:
...
class RefLocalTrackingModel031Manager(
        CommonModelManager,
        CommonModelManagerDictGeneratable[RefLocalTrackingModel031ArgumentsDict]):
    def __init__(
            self,
        ...
        __model_default_args = get_default_args(self.ref_local_tracking_model_031)
        __model_default_values = RefLocalTrackingModel031Arguments(__model_default_args)
    ...
    @classmethod
    def init_with_dict(cls, option_dict: Optional[RefLocalTrackingModel031ArgumentsDict] = None):
        if option_dict is not None:
            ref_local_tracking_model_031_arguments = RefLocalTrackingModel031Arguments(option_dict)
            return cls(
    ...
    def init_with_str_dict(cls, option_str_dict: Optional[Dict[str, str]] = None):
        if option_str_dict is not None:
            unet_level_arguments_dict = RefLocalTrackingModel031Arguments.convert_str_dict(option_str_dict)
            return cls.init_with_dict(unet_level_arguments_dict)
        else:
            return cls()
    
    def setup_model(self) -> Model:
        return self.ref_local_tracking_model_031(
