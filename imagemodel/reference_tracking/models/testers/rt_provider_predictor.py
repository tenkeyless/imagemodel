...
class RTProviderPredictor(Predictor):
    def __init__(
            self,
            model: Model,
    ...
    def predict(self):
        for predict_data in self.predict_dataset:
            # predict_data = ((main_image, ref_image, ref_label_with_bin), (filenames, bin_color_map))
            # predicted = (main_bw_label, ref_bw_label, predicted_main_label_with_bin)
            predicted = self.model.predict(predict_data[0], verbose=1)
            predicted_current_bin_label = predicted[2]
            
            bin_color_map = predict_data[1][1]
            current_filenames = predict_data[1][0]
